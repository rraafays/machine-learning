...
class SafeImageFolder(torchvision.datasets.ImageFolder):
    def __getitem__(self, index):
        try:
            ...
            print(f"Could not read image {self.imgs[index][0]}: {e}")
            return None, None
...
# Create DataLoaders with filtering of None entries
train_loader = DataLoader(
    [(img, label) for img, label in train_data if img is not None],
    batch_size=32, shuffle=True
)
val_loader = DataLoader(
    [(img, label) for img, label in val_data if img is not None],
...
class ArtCNN(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Linear(256, len(train_data.classes))  # Number of output classes
        self.relu = nn.ReLU()
